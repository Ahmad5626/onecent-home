$(document).ready(function() {
    var offsetGap = 130; // Adjust this value for the gap

    // Smooth scroll to section on link click with offset
    $('.proj_tabs a').on('click', function(e) {
        e.preventDefault();
        ...
        if (targetElement) {
            window.scrollTo({
                top: targetElement.offsetTop - offsetGap,
                behavior: 'smooth'
            });
        }
    });

    // Change active class on scroll, considering the offset gap
    $(window).on('scroll', function() {
        var scrollPos = $(window).scrollTop() + offsetGap; // Add the offset gap to scroll position
        $('.proj_tabs a').each(function() {
            var currLink = $(this);
            var refElement = $(currLink.attr('href'));
            
            if (refElement.position().top <= scrollPos && refElement.position().top + refElement.height() > scrollPos) {
                $('.proj_tabs li').removeClass('active');
...
(function ($) {
    ...
    $("#sidebarActive").on("click", function () {
        if (window.innerWidth > 0 && window.innerWidth <= 991) {
            $(".reallow-sidebar, .app-header, .app-page-body").toggleClass("open");
        } else {
            $(".reallow-sidebar, .app-header, .app-page-body").toggleClass("collapsed");
        }
        $(".app-offcanvas-overlay").toggleClass("overlay-open");
    });
    $(".app-offcanvas-overlay").on("click", function () {
        $(".reallow-sidebar, .app-header, .app-page-body").removeClass("collapsed");
        $(".reallow-sidebar, .app-header, .app-page-body").removeClass("open");
    ...
    let reviewStarElm = $(".reallow-ratings-two");
